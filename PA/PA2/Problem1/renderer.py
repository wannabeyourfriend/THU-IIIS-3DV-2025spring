...
class VolumeRenderer(torch.nn.Module):
    ...
    def _compute_weights(
        ...
        acc_density_delta = torch.cumsum(rays_density * deltas, dim=1)
        shifted_density_delta = torch.cat([torch.zeros((N, 1, 1), device=rays_density.device), acc_density_delta[:, :-1, :]], dim=1)
        T = torch.exp(-shifted_density_delta + eps)
