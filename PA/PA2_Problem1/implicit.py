...
class NeuralRadianceField(torch.nn.Module):
    def __init__(
        ...
        self.encoder = MLPWithInputSkips(n_layers=6, input_dim=embedding_dim_xyz, output_dim=0, skip_dim=embedding_dim_xyz, hidden_dim=256, input_skips=[3])
        self.density_decoder = torch.nn.Sequential(
            torch.nn.Linear(256, 1),
            torch.nn.ReLU()
        )
        self.color_decoder = torch.nn.Sequential(
            torch.nn.Linear(256 + embedding_dim_dir, 128),
            torch.nn.ReLU(),
    ...
    def forward(self, ray_bundle):
        ...
        views = ray_bundle.directions.unsqueeze(1).expand(ray_bundle.sample_points.shape).reshape(-1, 3)
        point_embedding = self.harmonic_embedding_xyz(sample_points)
        view_embedding = self.harmonic_embedding_dir(views)

        f = self.encoder(point_embedding, point_embedding)
        density = self.density_decoder(f)
        color = self.color_decoder(torch.cat((f, view_embedding), dim=-1))
